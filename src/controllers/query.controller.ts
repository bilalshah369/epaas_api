...
export async function listQueries(req: Request, res: Response, next: NextFunction) {
  try {
    const queries = await getQueriesForApplication(req.params.id);
    res.json({ queries });
  } catch (e) { next(e); }
}
...
export async function addQuery(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const query = await createQuery(req.params.id, req.user!.userId, text, req.user!.roleCode);
...
export async function respond(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const query = await respondToQuery(req.params.qid, req.user!.userId, response);
    res.json({ query });
  } catch (e) { next(e); }
}
...
export async function nodalForward(req: Request, res: Response, next: NextFunction) {
  try {
    const query = await nodalForwardQueryToApplicant(req.params.qid);
    res.json({ query });
  } catch (e) { next(e); }
}
...
export async function nodalForwardResponse(req: Request, res: Response, next: NextFunction) {
  try {
    const query = await nodalForwardResponseToTech(req.params.qid);
    res.json({ query });
  } catch (e) { next(e); }
}
